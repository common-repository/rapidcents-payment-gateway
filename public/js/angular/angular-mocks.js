	...
	function routeToRegExp(path, opts) {
			...
			angular.mock.$Browser           = function ($log, $$taskTrackerFactory) {
				...
				self.defer.verifyNoPendingTasks = function (taskType) {
					...
					if (pendingTasks.length) {
						...
						throw new Error(
							'Deferred tasks to flush (' + pendingTasks.length + '):\n  ' +
							formattedTasks
						);
					}
				};
								...
								angular.forEach(
									...
									function (log) {
										angular.forEach(
											...
											function (logItem) {
												errors.push(
													'MOCK $log (' + logLevel + '): ' + String( logItem ) + '\n' +
													(logItem.stack || '')
												);
											}
										);
			...
			*/
			...
			.config(
				['$provide', function ($provide) {
					...
					$provide.decorator(
						'$animate',
						['$delegate', '$timeout', '$browser', '$$rAF', '$animateCss', '$$animateJs',
			...
			function $httpBackend(method, url, data, callback, headers, timeout, withCredentials, responseType, eventHandlers, uploadEventHandlers) {
				...
				*/
				...
				$httpBackend.verifyNoOutstandingRequest = function (digest) {
					...
					if (responses.length) {
						var unflushedDescriptions = responses.map(
							function (res) {
								return res.description; }
						);
						throw new Error(
							'Unflushed requests: ' + responses.length + '\n  ' +
		...
		function MockHttpExpectation(
			...
			this.match = function (method, url, data, headers) {
				...
				if (angular.isDefined( data ) && ! this.matchData( data )) {
					return false;
				}
				...
				this.matchUrl = function (url) {
					...
					if (angular.isFunction( expectedUrl.test )) {
						return expectedUrl.test( url );
					}
					if (angular.isFunction( expectedUrl )) {
				...
				this.matchHeaders = function (headers) {
					if (angular.isUndefined( expectedHeaders )) {
						return true;
					}
					if (angular.isFunction( expectedHeaders )) {
						return expectedHeaders( headers );
					}
					return angular.equals( expectedHeaders, headers );
				};

				this.matchData = function (data) {
					if (angular.isUndefined( expectedData )) {
						return true;
					}
					if (expectedData && angular.isFunction( expectedData.test )) {
						return expectedData.test( data );
					}
					if (expectedData && angular.isFunction( expectedData )) {
			...
			angular.mock.$TimeoutDecorator = ['$delegate', '$browser', function ($delegate, $browser) {
				...
				$delegate.verifyNoPendingTasks = function () {
					...
					if (pendingTasks.length) {
						...
						var hasPendingTimeout = pendingTasks.some(
							function (task) {
            ...
            function ComponentControllerProvider($compileProvider) {
                this.$get = ['$controller','$injector', '$rootScope', function ($controller, $injector, $rootScope) {
                    return function $componentController(componentName, locals, bindings, ident) {
                        ...
                        var candidateDirectives = directives.filter(
			...
			(function () {
				...
				window.browserTrigger = function browserTrigger(element, eventType, eventData) {
					...
					if (/transitionend/.test( eventType )) {
						...
						try {
							evnt = new window.CompositionEvent(
								eventType,
								{
									data: eventData.data
								}
